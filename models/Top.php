...
class Top
{
    ...
    static function get_top($id){
        ...
        $requete = $bdd->prepare("SELECT tops.id 'top_id', tops.title 'to_title', tops.description 'to_description', tops.date, elements.title 'el_title', users.pseudo, users.id 'user_id', elements.description, elements.image_url 'el_image'
                                    FROM tops
